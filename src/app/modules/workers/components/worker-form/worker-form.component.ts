  ...
  async onSubmit() {
    ...
    try {
      ...
      if (this.mode() === 'create') {
        ...
        const result = await this.workersService.createWorker(workerData, 'current-user-uid');
        ...
        const result = await this.workersService.updateWorker(worker.id, updateData);
