// src/app/shared/modules/dynamic-form-builder/components/field-config-dialog/field-config-dialog.component.ts
...
export interface FieldConfigDialogData {
  mode: 'create' | 'edit';
  field?: FieldConfig;
  configService: ModuleConfigBaseService<any>;
  moduleName?: string;
}

/**
 * Componente genérico para configurar campos dinámicos
...
export class FieldConfigDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.moduleName = this.data.moduleName || 'registros';
    this.initializeForm();
  }
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (this.isEditMode && this.data.field) {
        ...
        await this.data.configService.addCustomField(newField as any);
